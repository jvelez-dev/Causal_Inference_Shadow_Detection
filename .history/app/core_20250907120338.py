...
def cpd_to_dataframe(cpd):
    """
    Convierte un TabularCPD de pgmpy en un DataFrame legible
    """
    # Columnas según las variables condicionales
    if cpd.variables[1:]:  
        col_names = pd.MultiIndex.from_product(
            [cpd.state_names[var] for var in cpd.variables[1:]],
            names=cpd.variables[1:]
        )
        df = pd.DataFrame(cpd.values, index=cpd.state_names[cpd.variable], columns=col_names)
    else:  
        # Si no tiene condicionantes (distribución marginal)
        df = pd.DataFrame(cpd.values.T, index=cpd.state_names[cpd.variable], columns=[cpd.variable])
