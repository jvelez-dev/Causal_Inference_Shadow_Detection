...
def causalidad(df: pd.DataFrame, modelo=None, tipo="ate"):
    ...
    if tipo == "ate":
        ...
        ate = model.estimates['ols']['ate']

        resultado = {
            "ATE": ate,
            "Resumen": model.estimates
        }
        return resultado
        ...
        return {"No hay Esfera(B)": df_do0, "Hay Esfera(B)": df_do1}
...
def detectar_sombra_threshold(image_path: str, min_area: int = 20):
    ...
    Devuelve el porcentaje de píxeles marcados en rojo.
    """
    ...
    for contour in contours:
        ...
        if area > min_area:
            cv2.drawContours(output_image, [contour], -1, (0, 0, 255), 2)  # Rojo en BGR

    # Contar píxeles rojos (0, 0, 255)
    red_mask = (output_image[:, :, 2] == 255) & (output_image[:, :, 1] == 0) & (output_image[:, :, 0] == 0)
    red_pixels = np.sum(red_mask)
    total_pixels = output_image.shape[0] * output_image.shape[1]
    porcentaje_rojo = (red_pixels / total_pixels) * 100
    ...
    if len(output_image.shape) == 3 and output_image.shape[2] == 3:
        ...
        image_rgb = output_image

    fig, ax = plt.subplots(figsize=(10, 8))
    ax.imshow(image_rgb, cmap="gray" if len(image_rgb.shape) == 2 else None)
    ax.set_title(f"Detección de Sombra - Threshold\nRojo: {porcentaje_rojo:.3f}% de la imagen")
    ax.axis("off")
    st.pyplot(fig)

    return porcentaje_rojo  
...
def detectar_sombra_adaptativo(image_path: str, min_area: int = 100):
    ...
    fig, ax = plt.subplots(figsize=(10, 8))
    ax.imshow(image_rgb, cmap="gray" if len(image_rgb.shape) == 2 else None)
    ax.set_title(f"Umbral Adaptativo\nRojo: {porcentaje_rojo:.3f}% de la imagen")
    ax.axis("off")
    st.pyplot(fig)
...
def detectar_sombra_felzenszwalb(image_path: str, scale: int = 100, sigma: float = 0.5, min_size: int = 50, umbral: float = 0.1):
    ...
    Devuelve el porcentaje de píxeles clasificados como sombra.
    """
    ...
    shadow_segments = np.where(np.array(segment_means) < umbral)[0]
    shadow_mask = np.isin(segments, shadow_segments)

    # Calcular porcentaje de sombra
    shadow_pixels = np.sum(shadow_mask)
    total_pixels = shadow_mask.size
    porcentaje_sombra = (shadow_pixels / total_pixels) * 100

    # Visualizar
    fig, ax = plt.subplots(figsize=(10, 8))
    ax.imshow(image)
    ax.contour(shadow_mask, colors="red", linewidths=2)
    ax.set_title(f"Segmentación Felzenszwalb\nSombra: {porcentaje_sombra:.3f}% de la imagen")
    ax.axis("off")
    st.pyplot(fig)

    return porcentaje_sombra  # ✅ Devuelve el porcentaje (de sombra, no de rojo)
