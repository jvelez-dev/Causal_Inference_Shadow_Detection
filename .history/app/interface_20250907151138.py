...
# ===========================================================
    ...
    for ind in independencias.get_assertions():
        var, independientes, condicion = ind[0], ind[1], ind[2]
        rows.append({"Variable": var, "Independiente de": independientes, "Condicionado en": condicion})

    import pandas as pd
    df_ind = pd.DataFrame(rows)

    st.subheader("📌 Independencias Condicionales del Modelo")
    st.dataframe(df_ind)
