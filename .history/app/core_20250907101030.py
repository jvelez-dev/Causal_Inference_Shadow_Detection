...
def do_intervention(modelo: DiscreteBayesianNetwork, variable: str, valor: int, objetivo: str):
    # Copiamos el modelo
    intervened_model = modelo.copy()

    # Quitamos las aristas que llegan a la variable intervenida
    parents = list(intervened_model.predecessors(variable))
    for p in parents:
        intervened_model.remove_edge(p, variable)

    # Fijamos la CPD de la variable intervenida
    from pgmpy.factors.discrete import TabularCPD
    cpd = TabularCPD(variable=variable, variable_card=2, values=[[1,0]] if valor==0 else [[0,1]])
    intervened_model.add_cpds(cpd)

    # Hacemos inferencia sobre el modelo intervenido
