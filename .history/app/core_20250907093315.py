...
def detectar_sombra_threshold(image_path: str, min_area: int = 20):
    ...
    plt.title("Detección de Sombra (Threshold Fijo)")
    plt.axis("off")
    plt.show()
...
def detectar_sombra_adaptativo(image_path: str, min_area: int = 100):
    ...
    plt.figure(figsize=(10, 8))
    plt.imshow(image_rgb)
    plt.title("Detección de Sombra (Umbral Adaptativo)")
    plt.axis("off")
    plt.show()
...
def detectar_sombra_felzenszwalb(image_path: str, scale: int = 100, sigma: float = 0.5, min_size: int = 50, umbral: float = 0.1):
    ...
    # Si la imagen tiene canal alfa, se elimina
    if image.shape[-1] == 4:
        image = image[:, :, :3]
    ...
    # Medias de intensidad por segmento
    segment_means = []
    for segment_id in np.unique(segments):
        segment_mask = (segments == segment_id)
        segment_mean = np.mean(gray_image[segment_mask])
        segment_means.append(segment_mean)

    shadow_segments = np.where(np.array(segment_means) < umbral)  # segmentos oscuros
    shadow_mask = np.isin(segments, shadow_segments)

    plt.figure(figsize=(10, 8))
    plt.imshow(image)
    plt.contour(shadow_mask, colors="red", linewidths=2)
    plt.title("Detección de Sombra (Felzenszwalb)")
    plt.axis("off")
    plt.show()
